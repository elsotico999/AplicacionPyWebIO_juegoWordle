...
th>div,td>div {width:50px; height:50px}.btn-light {background-color:#1AE3E5;}
    ...
    if len(session_local.curr_word) == WORD_LEN:  # submit a word guess
        for idx, c in enumerate(session_local.curr_word):
            ...
            if TODAY_WORD[idx] == c:
                ...
                run_js('$("button:contains(%s)").css({"background-color":"#34E51A", "color":"white"})' % c)
                text_bg = '#34E51A'
...
def main():
    put_table([
    [style(put_text('WORDLE').style('text-align: center'), 'color:orange')],
    
    [style(put_text('A pure python implementation of Wordle game, by using PyWebIO library').style('text-align: center'), 'color:red')]])
